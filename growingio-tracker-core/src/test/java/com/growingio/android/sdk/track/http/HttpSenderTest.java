...
public class HttpSenderTest {
    ...
    public void sendTest() {
        ...
        List<GEvent> events = Collections.singletonList(new CustomEvent.Builder().build());
        Truth.assertThat(eventHttpSender.send(events).isSucceeded()).isFalse();
        TrackerContext.get().getRegistry()
                .register(EventUrl.class, EventResponse.class, new TestModelFactory<>(new EventResponse(true)));

        TrackerContext.get().getRegistry().register(EventData.class, EventStream.class, new TestModelFactory<>(new EventStream(eventHttpSender.defaultDataTransfer(events))));
        ...
        SendResponse response = eventHttpSender.send(events);
