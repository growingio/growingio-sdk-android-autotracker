...
public class CircularFifoQueue<E> extends AbstractCollection<E>
    ...
    private void readObject(final ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        mElements = (E[]) new Object[mMaxElements];
        final int size = in.readInt();
        for (int i = 0; i < size; i++) {
            mElements[i] = (E) in.readObject();
        }
        mStart = 0;
        mFull = size == mMaxElements;
        if (mFull) {
            mEnd = 0;
        } else {
            mEnd = size;
    ...
    public int size() {
        ...
        if (mEnd < mStart) {
            size = mMaxElements - mStart + mEnd;
        } else if (mEnd == mStart) {
            size = mFull ? mMaxElements : 0;
        } else {
            size = mEnd - mStart;
        }
    ...
    public E get(final int index) {
        ...
        final int idx = (mStart + index) % mMaxElements;
    ...
    public E remove() {
        ...
        final E element = mElements[mStart];
        if (null != element) {
            mElements[mStart++] = null;

            if (mStart >= mMaxElements) {
                mStart = 0;
            }
            mFull = false;
        }
    ...
    public Iterator<E> iterator() {
        return new Iterator<E>() {
            ...
            private int mLastReturnedIndex = -1;
            private boolean mIsFirst = mFull;

            @Override
            public boolean hasNext() {
                return mIsFirst || mIndex != mEnd;
            }
            ...
            public E next() {
                ...
                mIsFirst = false;
                mLastReturnedIndex = mIndex;
                mIndex = increment(mIndex);
                return mElements[mLastReturnedIndex];
            }

            @Override
            public void remove() {
                if (mLastReturnedIndex == -1) {
                    throw new IllegalStateException();
                }

                // First element can be removed quickly
                if (mLastReturnedIndex == mStart) {
                    CircularFifoQueue.this.remove();
                    mLastReturnedIndex = -1;
                    return;
                }

                int pos = mLastReturnedIndex + 1;
                if (mStart < mLastReturnedIndex && pos < mEnd) {
                    // shift in one part
                    System.arraycopy(mElements, pos, mElements, mLastReturnedIndex, mEnd - pos);
                } else {
                    // Other elements require us to shift the subsequent elements
                    while (pos != mEnd) {
                        if (pos >= mMaxElements) {
                            mElements[pos - 1] = mElements[0];
                            pos = 0;
                        } else {
                            mElements[decrement(pos)] = mElements[pos];
                            pos = increment(pos);
                        }
                    }
                }
